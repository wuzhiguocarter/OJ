...
1. assert.h
    ...
    assert(expression) checks if value of expression is 0; if it's not 0,
    do nothing;if it is 0, when excuting code, stdout will print error info.
    when NDEBUG is defined,
    assert will be disabled.
...
4. file operation: 
    int fscanf ( FILE * stream, const char * format, ... );
        ...
        Whitespace character: the function will read and ignore 
    any whitespace characters encountered before the next 
    non-whitespace character (whitespace characters include
     spaces, newline and tab characters -- see isspace). 
     A single whitespace in the format string validates any 
     quantity of whitespace characters extracted from the 
